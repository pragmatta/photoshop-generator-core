...
var Generator = {
    ...
    /**
     * Get the layer depth in the document hierarchy with doc = 0, root layers = 1, etc.
     * @param {Object} layer Layer-object
     *
     * @return {number}
     *
    ...
    layerGetByIndex: function (doc_id, layer_index) {
        var doc = Generator.documentGetById(doc_id)
        if (doc && (layer_index || layer_index === 0)) {
            // Generator.logDebug("layerGetByIndex", "index=" + layer_index+", doc_id="+doc_id)
            return doc._layerOrderIndex[layer_index]
        }
    },
    /**
    ...
    layerGetNextLayer: function (layer) {
        if (layer) {
            ...
            if (order_index[layer_index])
                return order_index[layer_index]
            else
                return null // if no previous found, original was last
        }
    },
    /**
    ...
    layerGetPreviousLayer: function (layer) {
        if (layer) {
            ...
            if (order_index[layer_index])
                ...
                return null // if no previous found, original was first
        }
    },
    /**
    ...
    layerGetByUid: function (layer_uid) {
        if (layer_uid) {
            ...
            return Generator._layerUidIndex[layer_uid]
        }
    },
    /**
    ...
    _printOrderIndex: function (index) {
        var result = "["
        for (var i=0; i<index.length; i++)
            if (index[i])
                result += index[i].id + ' (' + index[i]._depth + '),'
            else
                result += '(null),'
    ...
    _generateOrderIndex: function (doc) {
        ...
        Generator.logDebug("_generateOrderIndex", "index="+Generator._printOrderIndex(order_index))
    ...
    _parseDocumentData: function (data) {
        ...
        if (!Generator._generateOrderIndex(data)) {
            Generator.logError("_parseDocumentData", "Unable to generate document order index, corrupt data!") // should not happen, when getting full data should be OK
        }
        ...
        if (layer) {
            ...
            if (delta.layers) {
                for (var i=0; i<delta.layers.length; i++) {
                    // Generator.logDebug("_parseLayerChanges", "layer_id="+delta._uid)
    ...
    _deleteLayerData: function (layer, recursive_call) {
        if (layer) {
            ...
            delete Generator._layerUidIndex[layer._uid]
